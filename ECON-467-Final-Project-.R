...
###1g 
nsim <- 1000L
sim <- bld.mbb.bootstrap(nyurn.train, nsim)
h <- 36L
future <- matrix(0, nrow=nsim, ncol=h)
for(i in seq(nsim))
  future[i,] <- simulate(ets(sim[[i]]), nsim=h)
start <- tsp(nyurn.train)[2]+1/12
simfc <- structure(list(
  ...
  level=95),
  class="forecast")
etsfc <- forecast(ets(nyurn.train), h=h, level=95)
autoplot(nyurn.train) +
  ggtitle("New York Unemployement Rate") +
  xlab("Year") + ylab("unemployment rate") +
  autolayer(simfc, series="Simulated") +
  autolayer(etsfc, series="ETS")
##The ETS prediction intervals produced using bootsraped 
##time series is much larger than ETS applied to the 
##original data. 

###1h 
sim <- bld.mbb.bootstrap(nyurn.train, 10) 
...
autoplot(nyurn.train) +
  ...
  guides(colour="none")
etsfc <- nyurn.train %>% ets() %>% forecast(h=48)
baggedfc <- debitcards %>% baggedETS() %>% forecast(h=48)
autoplot(debitcards) +
